#this script is intended to be used with dRMM as a Device Monitor

function Output-dRMM-Result{
    Param([string]$ods, [int16]$exitval)
    Write-Host "<-Start Result->"
    Write-Host "OneDriveStatus=$($ods)"
    Write-Host "<-End Result->"
    exit $exitval
}
#get version of OS - only run on Win10!
...
"@
    ...
    foreach ($ODStat in $ODStatus) {
        if ($ODStat.StatusString -in $ErrorList) { $ODerrors = $ODStat.StatusString }
    }
    if (!$ODerrors) {
        $ODerrors = "Healthy"
        Output-dRMM-Result -ods $ODerrors -exitval 0
    }else{
        Output-dRMM-Result -ods $ODerrors -exitval 1
    }
}else{
    Output-dRMM-Result -ods "INVALID OS" -exitval 0
}